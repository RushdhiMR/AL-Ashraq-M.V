...
if ($is_student) {
    $grade = $user['class_grade'] ?? 'Grade 10';
} elseif ($is_lecturer) {
...
if ($is_student) {
    $grade_safe = sanitize($conn, $grade);
    $result = mysqli_query($conn, "SELECT * FROM timetable_entries WHERE grade='$grade_safe' ORDER BY FIELD(day,'Monday','Tuesday','Wednesday','Thursday','Friday'), period_index");
} elseif ($is_lecturer) {
...
<div class="prt-panel" style="margin-bottom:16px;">
    <div class="prt-panel-head">
        <div class="prt-panel-title"><i class="fas fa-calendar-alt"></i> Weekly Timetable — <?php echo htmlspecialchars($grade); ?></div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="padding:8px 10px;border:1px solid var(--border);text-align:left;vertical-align:top;min-width:140px;">
                            ...
                            <div style="font-size:.82rem;color:var(--muted);"><?php echo htmlspecialchars($cell['teacher'] ?: 'Teacher not assigned'); ?></div>
                            <div style="font-size:.82rem;color:var(--muted);"><?php echo htmlspecialchars($cell['room'] ?: 'Room not assigned'); ?></div>
                            <?php else: ?>
                            <span style="color:var(--gray);font-size:.83rem;">Not scheduled</span>
                            <?php endif; ?>
                        </td>
                        <?php endforeach; ?>
                    </tr>
    ...
    <div class="prt-panel-foot" style="font-size:.75rem;color:var(--gray);">
        ...
        The timetable for your grade has not been configured yet. Please ask an administrator to update the schedule from the admin panel.
    </div>
    <?php endif; ?>
</div>
<?php else: ?>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="padding:8px 10px;border:1px solid var(--border);text-align:left;vertical-align:top;min-width:140px;">
                            <?php if ($cell): ?>
                            <div style="font-weight:700;color:var(--text);margin-bottom:4px"><?php echo htmlspecialchars($cell['subject']); ?></div>
                            <div style="font-size:.82rem;color:var(--muted);"><?php echo htmlspecialchars($cell['grade']); ?></div>
                            <div style="font-size:.82rem;color:var(--muted);"><?php echo htmlspecialchars($cell['room'] ?: 'Room not assigned'); ?></div>
                            <?php else: ?>
